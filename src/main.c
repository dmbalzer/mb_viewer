...
static bool show_device_window = false;
...
int main(void)
{
	...
	while ( !quit )
	{
		...
		if ( show_device_window ) show_device_window = !draw_device_window(NULL);
...
static void draw_main_menu(void)
{
	...
	if ( file_menu_active )
	{
		...
		/* New Device */
		if ( GuiLabelButton((Rectangle){ sub_menu_bounds.x, sub_menu_bounds.y + 24*i++, sub_menu_bounds.width, 24 }, "New Device") ) { file_menu_active = false; show_device_window = true; GuiDisable(); }
		/* Quit */
